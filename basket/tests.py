...
class BasketSessionTest(TestCase):
    ...
    def test_basket_session_persistence(self):
        ...
        self.assertIsNotNone(basket_id, "basket_id should be in session after ADD")

        # 2nd Request: REMOVE the item
        # Client maintains cookies, so should send sessionid back

        response = self.client.post(
            self.remove_url, data=json.dumps(payload), content_type="application/json"
        )
        ...
        print(f"[TEST_REMOVE] Session Key: {session_after.session_key}")
        print(f"[TEST_REMOVE] Basket ID in Session: {session_after.get('basket_id')}")
